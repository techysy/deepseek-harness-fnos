...
简单说：**装到 NAS 后，局域网内任何设备都能通过浏览器打开一个 DeepSeek 的 Agent 工作台**——不只是聊天，还能在你指定的工作空间里自主执行任务（写代码、跑命令、分析文件、操作仓库）。配置模型、插件、Agent 预设都在网页里完成。
...
如果你访问 GitHub / 外部 API 超时，安装向导或应用设置页都能填 **代理 IP + 端口**（例如 `127.0.0.1` + `7890`），会写入 `proxy.conf`，dsh 的网络请求（git/npm/API）就走代理。**留空默认不走代理**。
